...
export function setupMediaSession(activeTabId) {
  ...
  navigator.mediaSession.setActionHandler("play", () => {
    browser.tabs.sendMessage(activeTabId, { type: "SET_PLAYBACK", action: "play" });
  });
  navigator.mediaSession.setActionHandler("pause", () => {
    browser.tabs.sendMessage(activeTabId, { type: "SET_PLAYBACK", action: "pause" });
  });
  navigator.mediaSession.setActionHandler("seekforward", () => {
    browser.tabs.sendMessage(activeTabId, { type: "SET_PLAYBACK", action: "prev", time: (mediaStateCache.get(activeTabId)?.currentTime || 0) + 30 });
  });
  navigator.mediaSession.setActionHandler("seekbackward", () => {
    browser.tabs.sendMessage(activeTabId, { type: "SET_PLAYBACK", action: "next", time: Math.max(0, (mediaStateCache.get(activeTabId)?.currentTime || 0) - 10) });
  });
  navigator.mediaSession.setActionHandler("seekto", (details) => {
    if (details.seekTime) {
      browser.tabs.sendMessage(activeTabId, { type: "SET_PLAYBACK", action: "seek", time: details.seekTime });
    }
  });
}
...
export async function pollActiveTab() {
  try {
    const win = await new Promise((r) => browser.windows.getLastFocused(r));
    const tabs = await new Promise((r) => browser.tabs.query({ active: true, windowId: win.id }, r));
    if (!tabs[0]?.id) {
      LOG("poll: no active tab found");
      return;
    }
    const resp = await new Promise((r) => browser.tabs.sendMessage(tabs[0].id, { type: "GET_MEDIA_STATE" }, r));
    if (!resp) {
